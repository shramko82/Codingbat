...
public int countYZ(String str) {
	  ...
	  {
	  if (((Character.toLowerCase(ss[i]) == 'y') || (Character.toLowerCase(ss[i]) == 'z')) && (!Character.isLetter(ss[i+1])))
	  count++;
	  }
	  
	  if ((Character.toLowerCase(ss[ss.length-1]) == 'y') || (Character.toLowerCase(ss[ss.length-1]) == 'z'))
	  count++;
	  return count;
...
public boolean gHappy(String str) {
	  ...
	  if ((cc.length == 1)&&(cc[0] == 'g'))
	  return false;
	  
	  for(int i = 1; i < cc.length-1;i++)
	  if (cc[i] == 'g')
	  if ((cc[i-1] != 'g')&&(cc[i+1] != 'g'))
	  return false;
	  
	  if ((cc[0] == 'g')&&(cc[1] != 'g'))
	  return false;
	  
	  if ((cc[cc.length-1] == 'g')&&(cc[cc.length-2] != 'g'))
	  return false;
